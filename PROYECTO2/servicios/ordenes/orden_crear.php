...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!$id_paciente || !$id_medico || !$id_servicio) {
        ...
        $stmt = $pdo->prepare("INSERT INTO orden (id_paciente, id_medico, id_servicio, descripcion, estatus) VALUES (?, ?, ?, ?, ?)");
        $stmt->execute([$id_paciente, $id_medico, $id_servicio, $descripcion, $estatus]);
        header("Location: /PROYECTO2/servicios/ordenes/orden_listar.php?msg=Orden creada correctamente");
        exit;
    }
}
                ...
                <div class="col-md-6 mb-3 mb-md-0">
                    ...
                    <select name="id_paciente" id="id_paciente" class="form-select" required>
                        ...
                        <?php foreach($pacientes as $p): ?>
                            <option value="<?=$p['id_paciente']?>" <?=(@$_POST['id_paciente']==$p['id_paciente'])?'selected':''?>><?=htmlspecialchars($p['nombre'])?></option>
                        <?php endforeach;?>
                    </select>
                </div>
                <div class="col-md-6">
            ...
            <div class="row mb-3">
                ...
                <div class="col-md-6">
                    ...
                    <select name="estatus" id="estatus" class="form-select">
                        <option value="Pendiente" <?=(@$_POST['estatus'] ?? 'Pendiente')=='Pendiente'?'selected':''?>>Pendiente</option>
                        <option value="Realizada" <?=(@$_POST['estatus'] ?? '')=='Realizada'?'selected':''?>>Realizada</option>
                        <option value="Cancelada" <?=(@$_POST['estatus'] ?? '')=='Cancelada'?'selected':''?>>Cancelada</option>
                    </select>
                </div>
            </div>
            <div class="mb-3">
                <label for="descripcion" class="form-label">Descripción</label>
                <textarea name="descripcion" id="descripcion" class="form-control" rows="2"><?=htmlspecialchars(@$_POST['descripcion'] ?? '')?></textarea>
            </div>
            <div class="row mt-4">
                ...
                <div class="col-6 d-grid">
                    <button type="submit" class="btn btn-success btn-form">
                        <i class="bi bi-floppy"></i> Crear Orden
                    </button>
                </div>
            </div>
        </form>
